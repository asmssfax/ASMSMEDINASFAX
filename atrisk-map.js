...
PHOTO GALLERY
=====================================*/
// مهم: المتصفح لا يستطيع قراءة محتويات مجلد GitHub مباشرة، لذلك نستعمل
// ملف فهرس تلقائي للصور. أنشئ/حدّث images/atrisk/photos/photos-index.json
// عند إضافة صور جديدة. لا يوجد أي حد لعدد الصور.
let atriskGalleryRequestId = 0;
let atriskPhotosIndexPromise = null;

async function loadAtriskPhotosIndex(){
    if (!atriskPhotosIndexPromise) {
        atriskPhotosIndexPromise = fetch('images/atrisk/photos/photos-index.json', { cache: 'no-cache' })
            .then(r => r.ok ? r.json() : {})
            .catch(() => ({}));
    }
    return atriskPhotosIndexPromise;
}

function getAtriskPhotos(index, id){
    const padded = String(id).padStart(3, '0');
    const list = index[String(id)] || index[padded] || [];
    return list.map(file =>
        file.startsWith('http') || file.startsWith('/') || file.startsWith('images/')
            ? file
            : `images/atrisk/photos/${file}`
    );
}
...
async function renderAtriskGallery(building, propLabel){
    ...
    const index = await loadAtriskPhotosIndex();
    if (requestId !== atriskGalleryRequestId) return;

    const photos = getAtriskPhotos(index, building.id);
    if (photos.length === 0) {
    ...
    photos.forEach((src, photoIndex) => {
        ...
        img.src = src;
        img.loading = 'lazy';
        img.alt = `${propLabel} - ${photoIndex + 1}`;
        img.onerror = () => { thumb.remove(); };
        thumb.appendChild(img);
        thumb.addEventListener('click', () => openAtriskLightbox(photos, photoIndex, propLabel));
